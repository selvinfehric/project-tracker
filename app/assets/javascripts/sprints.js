...
APP.Sprints.init = function () {
	APP.Sprints.addEvents();
}

APP.Sprints.addEvents = function () {
	 ...
	 $( ".scrum_board_list" ).sortable({
	    ...
	    stop: function(event, ui) { APP.Sprints.onStoryMove(event, ui) }        
	});		
}

APP.Sprints.onStoryMove = function (event, ui) {
	...
	data.story = {};
	data.story.status_id = APP.Sprints.Status[status];		
	
	$.ajax({
		type : "put",
		data : data,
		url : APP.baseUrl + "stories/" + $this.attr("id") + ".json",
		dataType : "json",
		success : function (data) {
			//
		},
		error : function () { alert("Failed to update story status."); }		
	})
}
